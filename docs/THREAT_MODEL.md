...
> **Status:** Initial draft, v0.1.0-rc. This document describes the security posture of ZECK as of the v0.1.0 release candidate. It is a living document; revisit on every release and whenever a new attack surface is added.

## 1. Purpose and scope

ZECK is a single-use Zcash wallet **recovery** tool for ZecWallet Lite seeds. Its purpose is to take a 24-word BIP-39 seed phrase, scan the Zcash chain for funds derived under ZecWallet Lite's account layout, and sweep them to a modern wallet (Zashi/Zodl, YWallet) in a single session. It is not an everyday wallet.

This threat model covers:

- the desktop GUI (Tauri v2: HTML/CSS/JS frontend in WebView2/WKWebView/WebKitGTK + Rust backend)
- the CLI (`zeck-cli`)
- the shared core library (`zeck-core`)
...
|---|---|---|---|
| `zeck-gui` (Tauri shell + WebView) | 2 processes (Rust host + WebView renderer) | Rust + HTML/CSS/JS | Host process trusts WebView only via `invoke` IPC; WebView is sandboxed by the OS |
| `zeck-cli` | 1 process | Rust | Inherits the user's shell trust |
| `zeck-core` | (library) | Rust | — |
...
| Malware running as a *different* unprivileged user | Read processes / files of the ZECK user via OS bugs | OS-level — out of scope |
...
| T-S4 | Seed phrase leaks via clipboard | M | ⚠️ | ZECK never writes the seed to the clipboard. The recovery-report "Copy path" button (PR #53) only copies the file path. Users pasting their own seed in is bounded by the OS clipboard's lifetime. |
...
- Pre-Sapling (Sprout) note recovery — librustzcash dropped Sprout scanning long before this project began; ZWL seeds whose only funds are in Sprout notes (block <419,200, before October 2018) cannot be recovered via ZECK.
